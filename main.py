...
with tf.device('/device:GPU:0'):
    while stale_iterations < MAX_STALE_ITERATIONS:
        ...
        loss = 0
        for i in range(int(len(x_train_set) / BATCH_SIZE)):
            loss += model.test_on_batch(x=x_train_set[i*BATCH_SIZE:(i+1)*BATCH_SIZE], y=y_train_set[i*BATCH_SIZE:(i+1)*BATCH_SIZE])[LOSS]
        train_loss += [loss]

        valid = model.test_on_batch(x=x_valid_set, y=y_valid_set)[LOSS]
        valid_loss += [valid]
        test_loss += [model.test_on_batch(x=x_test_set, y=y_test_set)[LOSS]]
